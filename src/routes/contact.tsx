...
function Page() {
  ...
  // Use dynamic settings; fall back to i18n strings when not set.
    ...
    <>
      ...
      <Section>
        <div className="grid gap-14 lg:grid-cols-[1.5fr_1fr]">
            ...
            {done ? (
              <Reveal>
                <div className="card-flat p-10">
                  ...
                  <button
                    onClick={() => {
                      setDone(false);
                      setForm({ name: "", company: "", country: "", email: "", phone: "", businessType: "", inquiryType: "", message: "" });
                      setPicked([]);
                      setFileName("");
                    }}
              ...
              <form onSubmit={onSubmit} noValidate>
                ...
                <div className="mt-10 grid gap-6 sm:grid-cols-2">
                  {(
                    ...
                    <label key={k} className="grid gap-2">
                      ...
                      {errors[k] ? <span className="text-[0.78rem] text-red-600">{errors[k]}</span> : null}
                    </label>
                  ))}
                ...
                <label className="mt-8 grid gap-2">
                  ...
                  {errors["message"] ? <span className="text-[0.78rem] text-red-600">{errors["message"]}</span> : null}
                </label>
          ...
          {/* ── Contact Info Aside — reads from Supabase settings ── */}
          <Reveal>
            <aside className="card-flat p-8">
              ...
              <dl className="mt-6 grid gap-6">
                ...
                <div className="border-t border-hairline pt-4">
                  ...
                  <dd className="mt-2 text-[0.95rem] text-navy">{contactAddress}</dd>
                </div>
                {/* Email */}
                <div className="border-t border-hairline pt-4">
                  <dt className="text-[0.7rem] font-semibold tracking-[0.16em] uppercase text-science">
                  ...
                  <dd className="mt-2 text-[0.95rem] text-navy">
                    {contactEmail ? (
                      <a href={`mailto:${contactEmail}`} className="hover:text-science transition-colors">
                        {contactEmail}
                      </a>
                    ) : "—"}
                  </dd>
                </div>
                {/* Phone — shown only if set */}
                {contactPhone && (
                  <div className="border-t border-hairline pt-4">
                    <dt className="text-[0.7rem] font-semibold tracking-[0.16em] uppercase text-science">
                      Phone
                    ...
                    <dd className="mt-2 text-[0.95rem] text-navy">
                      <a href={`tel:${contactPhone}`} className="hover:text-science transition-colors">
                        {contactPhone}
                      </a>
                    </dd>
                  </div>
                ...
                <div className="border-t border-hairline pt-4">
                  ...
                  <dd className="mt-2 text-[0.95rem] text-navy">{contactHours}</dd>
                </div>
              </dl>
            </aside>
          </Reveal>
