...
export function Header() {
  ...
  // ── Logo: image if uploaded, text fallback otherwise ─────────────────────
  const logoUrl = settings?.logoUrl ?? "";
  const coName = locale === "ko"
    ? (settings?.companyNameKo || "Vesco Science")
    : (settings?.companyName || "Vesco Science");

  // Split on first space for two-tone text fallback
  const [firstWord, ...rest] = coName.split(" ");
  const secondPart = rest.join(" ");

  // ── Header is always light (white) background ─────────────────────────────
  // The logo is dark (navy) on white — always visible.
  // On scroll / open: add shadow for depth.
  const headerBg = scrolled || open || menu
    ? "bg-white shadow-[0_2px_16px_0_rgba(0,0,0,0.10)] border-b border-gray-100"
    : "bg-white border-b border-gray-100/80";

  // Nav text: dark navy on light bg
  ...
  const LogoContent = () =>
    logoUrl ? (
      ...
      <>
        <span className="font-display text-[1.05rem] font-bold tracking-[0.18em] text-navy uppercase">
          {firstWord}
        </span>
        {secondPart && (
    ...
    >
      <div className="mx-auto flex h-[72px] w-full max-w-[1240px] items-center justify-between px-6 md:px-10">
        ...
        <div className="flex items-center gap-4">
            ...
            {(["en", "ko"] as const).map((l) => (
              <button
                ...
                className={`px-3 py-1.5 text-[0.7rem] font-semibold tracking-[0.12em] uppercase transition-colors ${
                  locale === l ? "bg-teal text-[#05231f]" : "text-navy/60 hover:text-navy"
                }`}
              >
                {l === "en" ? "EN" : "KO"}
              </button>
          ...
          >
            <span className={`h-px w-5 bg-navy transition-transform duration-300 ${open ? "translate-y-[3.5px] rotate-45" : ""}`} />
            <span className={`h-px w-5 bg-navy transition-transform duration-300 ${open ? "-translate-y-[3.5px] -rotate-45" : ""}`} />
          </button>
        </div>
      </div>
